...
import { ArrowDown, ArrowUp, ArrowUpDown, Shield, GraduationCap, Book, Users, Eye, Fingerprint, ChevronsLeft, ChevronLeft, ChevronRight, ChevronsRight } from 'lucide-react'
            ...
            cell: ({ row }) => {
                ...
                const tooltipContent = (
                    <div className="flex flex-wrap gap-2">
                        {displayRoles.map(role => {
                            ...
                            return (
                                <Badge key={role} variant={c} className={cn('capitalize flex items-center', !order.includes(role) ? ' custom-role-badge' : '')}>
                ...
                const badgeContent = (
                    <span className="inline-flex items-center gap-1">
                        <Badge key={mainRole} variant={color} className={cn('capitalize flex items-center', !order.includes(mainRole) ? ' custom-role-badge' : '')}>
        ...
        <div className="relative flex flex-1 flex-col">
                                ...
                                {table.getRowModel().rows.map(row => (
                                    <TableRow key={row.id} className={cn('transition-opacity duration-300 border-b last:border-0 hover:bg-muted/40', loading ? 'opacity-50' : 'opacity-100')}>
                                        {row.getVisibleCells().map((cell, idx) => (
                                            <TableCell key={cell.id} className={cn(density === 'compact' ? 'py-1.5' : 'py-3', idx === 0 ? cn('pl-4', stickyId ? 'sticky left-0 z-[1] bg-inherit' : '') : '')} style={{ width: (cell.column.getSize?.() as number) || undefined }}>
            ...
            <div className="mt-4 flex min-h-[56px] items-center justify-between rounded-md border bg-card px-3 py-2 text-sm">
                ...
                <div className="flex items-center space-x-2">
                    <Button variant="outline" size="icon" className="hidden size-8 lg:flex" onClick={() => onPageChange(1)} disabled={currentPage <= 1}>
                        <span className="sr-only">Go to first page</span>
                        <ChevronsLeft />
                    </Button>
                    <Button variant="outline" size="icon" className="size-8" onClick={() => onPageChange(Math.max(1, currentPage - 1))} disabled={currentPage <= 1}>
                        <span className="sr-only">Go to previous page</span>
                        <ChevronLeft />
                    </Button>
                    <Button variant="outline" size="icon" className="size-8" onClick={() => onPageChange(Math.min(totalPages, currentPage + 1))} disabled={currentPage >= totalPages}>
                        <span className="sr-only">Go to next page</span>
                        <ChevronRight />
                    </Button>
                    <Button variant="outline" size="icon" className="hidden size-8 lg:flex" onClick={() => onPageChange(totalPages)} disabled={currentPage >= totalPages}>
